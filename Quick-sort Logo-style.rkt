...
(define all-same?
  (lambda (input-ls) 
    (if (empty? input-ls) 
        ...
        (let ((matcher (first input-ls)))
          (letrec ((helper (lambda (ls) 
                      (if (empty? ls) 
                          yes ; If the answer is yes, we return yes. And we are done. EXIT.
                          (if (same? (first ls) matcher) 
                              (helper (without-first ls)) 
...
(define quick-sort 
  (lambda (input-ls) 
    (if (empty? input-ls) 
        input-ls ; If the answer is yes, we return input-ls. And we are done. EXIT.
        (let ((pivot (first input-ls))) 
          (letrec ((helper 
                    (lambda (ls lower-part higher-part) 
                             (if (empty? ls) 
                                (combine ; If the answer is yes, we use combine to combine two lists into one list. And we are done. EXIT.
                                  ...
                                  (add-to-front pivot 
                                               (process higher-part))) 
                                (let ((firsty (first ls))) 
                                   (if (less-than? firsty pivot) 
                                       (helper (without-first ls) 
                                               (add-to-end lower-part firsty) 
                                               higher-part) 
                                       (helper (without-first ls)
                                               lower-part 
                                               (add-to-end higher-part firsty)))))))) 
            (helper (without-first input-ls) empty-list empty-list)))))) ; This last line is our initialization line.
                            ; We run helper with its original inputs.
